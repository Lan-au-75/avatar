...
function BookmarkProvider({ children }: { children: React.ReactNode }) {
    ...
    const handleBookmark = async (movie: Movie) => {
        ...
        toast.success(<ToastMessage status='Success' message='You have successfully saved the movie' />, {
    ...
    const handleRemoveBookmark = async (movie: Movie) => {
        ...
        toast.success(<ToastMessage status='Success' message='You have successfully delete the movie' />, {
            icon: <AiFillCheckCircle className='text-green-500 text-lg md:text-xl text-center' />,
        })
    }

    return (
        <BookmarkContext.Provider value={{ handleBookmark, handleRemoveBookmark }}>
            {' '}
            {children}
